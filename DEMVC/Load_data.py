...
def Get_MNIST_USPS_COMIC(datapath):
    data = 0
    if data == 1:
        x = scio.loadmat(datapath + "/MNIST-USPS.mat")
        print(x)
        x1 = x['X1']
        x2 = x['X2']
        Y = x['Y']
        ...
        print(x_shuffle_u[0])
        # print(y_shuffle[0])
        scio.savemat(datapath + '/MNIST_USPS.mat', {'X1': x_shuffle_m, 'X2': x_shuffle_u, 'Y': y_shuffle})
    data = scio.loadmat(datapath + "/MNIST_USPS.mat")
    x1 = data['X1']
    x2 = data['X2']
    Y = data['Y'][0]
...
def BDGP(datapath):
    data = scio.loadmat(datapath + "/BDGP.mat")
    x1 = data['X1']
    x2 = data['X2']
    Y = data['Y'][0]
    print(x1.shape)
...
def load_data_conv(dataset, datapath = "./data/"):
    print("load:", dataset)
    if dataset == 'MNIST_USPS_COMIC':             # MNIST-USPS
        return Get_MNIST_USPS_COMIC(datapath)
    elif dataset == 'BDGP':                       # BDGP
        return BDGP(datapath)
    else:
