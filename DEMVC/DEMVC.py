...
class MvDEC(object):
        ...
        if y is not None and verbose > 0:
            class PrintACC(callbacks.Callback):
                ...
                def on_epoch_end(self, epoch, logs=None):
                    ...
                    NMI, ARI, ACC, PUR = Nmetrics.evaluate(self.y, y_pred)
                    print("{:.4f}\t{:.4f}\t{:.4f}".format(ACC, NMI, PUR))
                    print('\n' + ' '*8 + '|==>  acc: %.4f,  nmi: %.4f  <==|'% (ACC,NMI))
    ...
    def fit(self, arg, x, y, maxiter=2e4, batch_size=256, tol=1e-3,
        ...
        for view in range(len(x)):
            ...
            nmi = np.round(NMI, 5)
            ari = np.round(ARI, 5)
            print('Start-'+str(view+1)+': acc=%.5f, nmi=%.5f, ari=%.5f' % (acc, nmi, ari))
            ...
            if ite % update_interval == 0:
                ...
                if y is not None:
                    ...
                    for view in range(len(x)):
                        ...
                        nmi = np.round(NMI, 5)
                        ari = np.round(ARI, 5)
                        logdict = dict(iter=ite, nmi=nmi, ari=ari, loss=avg_loss[view])
                        logwriter.writerow(logdict)
                        logfile.flush()
                        print('V'+str(view+1)+'-Iter %d: acc=%.5f, nmi=%.5f, ari=%.5f; loss=%.5f' % (
